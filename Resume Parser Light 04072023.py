...
def getParsedData():
    ...
    academic_project = get_academic_project(document)

    language = get_languages(document)
...
def get_academic_project(document):
    ...
    project_flag = False
    for data in document:
        if(data.isupper() and data in labels):
            project_flag = True
            continue
...
def pdf_table(file, nlp):
    ...
    with pdfplumber.open(file) as pdf:
        ...
        for line in temp:
            line = line.strip()
            non_ascii_line = remove_non_ascii(line)
            if line != '' and len(non_ascii_line.strip())>1:
                result.append(non_ascii_line)
